...
/**
 * Binds --pointer-x/--pointer-y CSS custom properties on an element, updated
 * only while the pointer is inside it (onPointerMove is a React event
 * handler scoped to that element — never a global window listener), for
 * CSS-driven pointer-aware effects like RivaCard's halo. Writes via
 * ref.style directly instead of React state so movement never triggers a
 * re-render.
 */
